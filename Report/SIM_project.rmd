...
for (i in 1:nrow(cor_matrix)) {
  for (j in 1:ncol(cor_matrix)) {
    if ((cor_matrix[i, j] > 0.4 || cor_matrix[i, j] < -0.4) && i != j) {
      cat(col_names[i], "-", col_names[j], "correlation:", cor_matrix[i, j], "\n")
    }
  }
}

# Mirar cuales son las variables con correlacion mas alta entre ellas y agruparlas para buscar Moutliers entre ellas
...
res.out = Moutlier(df_clean[,num_outliers], quantile = 0.9995, col="green")
